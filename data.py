...
class NiaDataset(Dataset):
    RANGE_TO_MEDIAN = {'a': (1 + 6) / 2, 'b': (7 + 12) / 2, 'c': (13 + 19) / 2, 'd': (20 + 30) / 2, 'e': (31 + 45) / 2,
                       'f': (46 + 55) / 2, 'g': (56 + 66) / 2, 'h': (67 + 80) / 2, 'above': 90}
    ...
    def __getitem__(self, idx: int):
        ...
        if self.transform:
            img = self.transform(img)
        sample = {'image': img, 'age': age, 'age_class': age_class, 'file': files, 'data_type': data_type}
        return sample
