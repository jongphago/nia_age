...
def train(
    ...
    for i, sample in enumerate(train_loader):
        ...
        if (i + 1) % interval == 0:
            print(
                "[%d, %5d] mean_loss: %.3f, variance_loss: %.3f, softmax_loss: %.3f, loss: %.3f"
                % (
                    epoch,
                    i,
                    running_mean_loss / interval,
...
def get_args():
    ...
    parser.add_argument(
        "-i", "--train_meta_path", type=str, default="nia_cropped/train_0.npy"
    )
    parser.add_argument(
        "-v", "--test_meta_path", type=str, default="nia_cropped/test_0.npy"
    )
    ...
    if epoch > 0:
        ...
        for epoch in range(epoch):
            ...
            if is_mean_variance:
                ...
                train_softmax(
                    train_loader, model, criterion2, optimizer, epoch, result_directory
                )
